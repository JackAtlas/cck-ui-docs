---
title: CCK UI PostCSS preset
lang: en-US
---

# CCK UI PostCSS preset

`postcss-preset-cck` provides several CSS functions and mixins to help you write styles. It is not required to use it, but is highly recommended. All demos that feature styles assume that you have this preset installed.

`postcss-preset-cck` includes the following PostCSS plugins:

- [postcss-nested](https://www.npmjs.com/package/postcss-nested)
- [postcss-mixins](https://www.npmjs.com/package/postcss-mixins) with CCK UI specific mixins
- Custom plugin with `em / rem` functions

## Installation

Install `postcss-preset-cck` as a dev dependency:

```bash
pnpm add --dev postcss-preset-cck
```
...
`rem` and `em` functions can be used to convert pixels to rem/em units. `16px = 1rem` and `16px = 1em`. `em` values are supposed to be used in media queries, `rem` everywhere else. You can learn more about unit conversion in [this guide](./rem).
...
```scss
.demo {
  font-size: calc(1rem * var(--c-scale));

  @media (min-width: 20em) {
    font-size: calc(2rem * var(--c-scale));
  }
}
```

## Auto convert to px to rem
...
```css
[data-c-color-scheme='light'] .demo {
  color: red;
}

[data-c-color-scheme='dark'] .demo {
  color: blue;
}
```
...
// In smaller-than mixin 0.1px is substracted from breakpoint value
// to avoid intersaction with larger-than mixin
...
You can also use `smaller-than` and `larger-than` mixins with cck ui breakpoints:

```scss
.demo {
  @mixin smaller-than $c-breakpoint-sm {
    color: red;
  }

  @mixin larger-than $c-breakpoint-sm {
    color: blue;
  }
}
```
...
`light-dark` function is an alternative to `light` and `dark` mixins. It accepts two arguments: first argument is rude that will be applied in light color scheme, second argument is rule that will be applied in dark color scheme.
...
`alpha` function can be used to add alpha channel to color. Note that it uses [color-mix](https://caniuse.com/mdn-css_types_color_color-mix) which is not supported in some old browsers.

```css
.demo {
  color: alpha(var(--c-color-red-4), 0.5);
...
```css
.demo {
  color: color-mix(in srgb, var(--c-color-red-4), transparent 50%);
...
```css
.demo {
  color: lighten(var(--c-color-red-4), 0.5);
...
```css
.demo {
  color: color-mix(in srgb, var(--c-color-red-4), white 50%);
...
```css
:where([data-c-color-scheme='light']) .demo {
  color: red;
}
```
...
You can define custom mixins that are not included in the preset by specifying them in the `mixins` option. To learn about mixins syntax, follow [postcss-mixins documentation](https://github.com/postcss/postcss-mixins).
